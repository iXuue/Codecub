...
class Pico:
    ...
    def emit_trace(self, task_state, event, payload=None):
        ...
        # Trace ownership remains in RunStore.  The event bus is fan-out only,
        # preventing the same lifecycle event from being persisted twice.
        self.event_bus.emit(
            self._canonical_event_name(event),
            run_id=getattr(task_state, "run_id", ""),
            agent_id=self.session.get("id", ""),
            payload=payload,
        )
        return payload
    ...
    def ask(self, user_message, run_id=""):
        ...
        prior_operations = {}
        state_path = self.run_store.task_state_path(task_run_id)
        if state_path.exists():
            prior_operations = dict(
                self.run_store.load_task_state(task_run_id).get("side_effect_operations", {}) or {}
            )
        elif self.current_checkpoint():
            prior_operations = dict(
                self.current_checkpoint().get("side_effect_operations", {}) or {}
            )
        task_state.side_effect_operations = prior_operations
        ...
        while True:
                ...
                if native_mode:
                    ...
                    if pending_native_calls:
                        ...
                        if self.context_compiler is not None:
                            self.working_state.refresh_fact_freshness(self.root)
                            memory_layer, memory_meta = self._memory_layer()
                            compiled_native, compiler_meta = (
                                self.context_compiler.compile_native(
                                    user_message,
                                    working_state=self.working_state,
                        ...
                        raw = self.completion_client.complete_with_tools(
                            native_messages,
                            native_tools,
                            self.max_new_tokens,
                    ...
                    raw = self.completion_client.stream_complete(
                        prompt,
                        self.max_new_tokens,
                    ...
                    raw = self.completion_client.complete(
                        prompt,
                        self.max_new_tokens,
                        **model_kwargs,
                    )
            ...
            completion_metadata = dict(
                getattr(self.completion_client, "last_completion_metadata", {}) or {}
            )
            if self.cancellation_requested(task_state):
                return self.stop_user_canceled_run(
                    task_state, run_started_wall, run_started_at
            ...
            if native_mode and not getattr(raw, "tool_calls", ()):
                recovered_call, rejection_reason = self._recover_native_text_tool_call(
                    getattr(raw, "text", raw)
                )
                if recovered_call is not None or rejection_reason is not None:
                    self.current_planning["legacy_tool_recovery_attempts"] = (
                        self.current_planning.get("legacy_tool_recovery_attempts", 0) + 1
                    )
                    if recovered_call is None:
                        self.current_planning["legacy_tool_recovery_rejected"] = (
                            self.current_planning.get("legacy_tool_recovery_rejected", 0)
                            + 1
                        )
                        self.emit_trace(
                            task_state,
                        ...
                        self.current_planning["legacy_tool_recovery_success"] = (
                            self.current_planning.get("legacy_tool_recovery_success", 0)
                            + 1
                        )
                        self.emit_trace(
                            task_state,
                        ...
                        tool_call_source = "legacy_recovered"
            if getattr(raw, "tool_calls", ()):
                self.current_planning["native_tool_calls"] = (
                    self.current_planning.get("native_tool_calls", 0)
                    + len(raw.tool_calls)
                )
            if native_mode:
                if raw.tool_calls:
            ...
            if kind == "tool":
                ...
                result = self.run_tool(name, args, operation_key=operation_key)
                if read_notice:
    ...
    def run_tool(self, name, args, operation_key=""):
        ...
        try:
            ...
            return message
        read_coverage = None
        read_range_action = "read"
        read_range_version = ""
        if name == "read_file":
            read_coverage, read_range_version = self.assess_read_range(args)
            covered_lines = sum(end - start + 1 for start, end in read_coverage.covered_ranges)
            if read_coverage.covered_ranges:
                self.current_planning["read_guard_triggered"] = (
                    self.current_planning.get("read_guard_triggered", 0) + 1
                )
                if bool(args.get("force", False)):
                    read_range_action = "forced"
                    self.current_planning["redundant_read_forced"] = (
                        self.current_planning.get("redundant_read_forced", 0) + 1
                    )
                elif read_coverage.fully_covered:
                    self.current_planning["redundant_read_suppressed"] = (
                        self.current_planning.get("redundant_read_suppressed", 0) + 1
                    )
                    self.current_planning["read_guard_covered_lines_skipped"] = (
                        self.current_planning.get("read_guard_covered_lines_skipped", 0)
                        + covered_lines
                    )
                    self._last_tool_result_metadata = {
                        "tool_status": "ok",
                        ...
                        "read_range_guard": {
                            ...
                            "covered_ranges": [list(item) for item in read_coverage.covered_ranges],
                            "returned_ranges": [],
                            "file_changed": False,
                            "action": "suppressed",
                        },
                    }
                    return self.redundant_read_notice(args, read_coverage, "suppressed")
                else:
                    read_range_action = "delta_read"
                    delta_lines = sum(
                        end - start + 1 for start, end in read_coverage.uncovered_ranges
                    )
                    self.current_planning["read_guard_delta_reads"] = (
                        self.current_planning.get("read_guard_delta_reads", 0) + 1
                    )
                    self.current_planning["read_guard_delta_lines_returned"] = (
                        self.current_planning.get("read_guard_delta_lines_returned", 0)
                        + delta_lines
                    )
                    self.current_planning["read_guard_covered_lines_skipped"] = (
                        self.current_planning.get("read_guard_covered_lines_skipped", 0)
                ...
                (item.path if isinstance(item, ReadRangeRecord) else item["path"])
                == task_policy.canonical_path(args.get("path"))
                for item in self.current_planning.get("read_range_ledger", [])
            ):
                read_range_action = "stale_read"
        ...
        if tool.get("circuit_breaker", True) and not self.tool_circuit_breaker.allow(
            name
        ):
            self._last_tool_result_metadata = {
                "tool_status": "blocked",
                "tool_error_code": "circuit_open",
                "circuit_state": self.tool_circuit_breaker.status(name),
                "risk_level": "high" if tool["risky"] else "low",
                "read_only": not tool["risky"],
                "affected_paths": [],
                "workspace_changed": False,
            ...
            return f"error: tool {name} circuit is open"
        side_effect_operation = None
        if bool(tool.get("side_effect")) and operation_key and self.current_task_state is not None:
            args_digest = self._side_effect_args_digest(args)
            claim = self.run_store.claim_side_effect_operation(
                self.current_task_state, operation_key, name, args_digest
            )
            if not claim["claimed"]:
                prior_state = claim.get("prior_state", "")
                if claim.get("conflict"):
                    error_code = "idempotency_key_conflict"
                    message = "error: idempotency key conflicts with a different side-effect operation"
                elif prior_state == "completed":
                    error_code = "side_effect_replay_blocked"
                    message = "error: side-effect operation was already completed"
                else:
                    error_code = "outcome_uncertain"
                    message = "error: prior side-effect outcome is uncertain; replay is blocked"
                self._last_tool_result_metadata = {
                    "tool_status": "blocked",
                    "tool_error_code": error_code,
                    "tool_execution_success": False,
                    "tool_business_success": False,
                    "tool_result_validation_failed": False,
                    "tool_result_failure_reason": "",
                ...
                return message
            side_effect_operation = {"key": operation_key, "args_digest": args_digest}
        before_snapshot = self.capture_workspace_snapshot() if tool["risky"] else {}
        after_snapshot = before_snapshot
        try:
            if name == "read_file" and read_range_action == "delta_read":
                delta_results = []
                for start, end in read_coverage.uncovered_ranges:
                    delta_args = {**args, "start": start, "end": end}
                    delta_results.append(clip(tool["run"](delta_args)))
                covered = ", ".join(
                    f"L{start}-L{end}" for start, end in read_coverage.covered_ranges
                )
                returned = ", ".join(
                    f"L{start}-L{end}" for start, end in read_coverage.uncovered_ranges
                )
                result = (
                    f"Read guard: {covered} was already available and unchanged. "
                    f"Only previously unseen {returned} is returned below.\n\n"
                    + "\n\n".join(delta_results)
                )
            else:
            ...
            tool_status = "ok"
            tool_error_code = ""
            business_success, failure_reason = toolkit.validate_tool_result_contract(
                tool, result
            )
            validation_failed = not business_success
            if validation_failed:
                tool_status = "partial_success" if workspace_changed else "error"
                # Keep run_shell's established status/error-code contract.
                if failure_reason == "nonzero_exit_code":
                    tool_error_code = (
                        "tool_partial_success" if workspace_changed else "tool_failed"
                    )
                else:
                    tool_error_code = (
                        "tool_partial_success" if workspace_changed else "tool_business_failure"
                    )
            self.update_memory_after_tool(name, args, result)
            self._last_tool_result_metadata = {
                ...
                "tool_result_validation_failed": validation_failed,
            ...
            if side_effect_operation is not None:
                self._last_tool_result_metadata.update(
                    {
                        "side_effect_operation_key": side_effect_operation["key"],
                        "side_effect_args_digest": side_effect_operation["args_digest"],
                        "side_effect_claimed": True,
                        "side_effect_replay_detected": False,
                        "side_effect_replay_blocked": False,
                        "side_effect_prior_state": "",
                        "side_effect_commit_recorded": business_success,
                        "side_effect_outcome_uncertain": not business_success,
                        "idempotency_key_conflict": False,
                    }
                )
                self.run_store.update_side_effect_operation(
                    self.current_task_state,
                    side_effect_operation["key"],
                    "completed" if business_success else "uncertain",
                    self._side_effect_result_metadata(self._last_tool_result_metadata),
                )
            if name == "read_file":
                self.record_read_range(
                    args, getattr(getattr(self, "current_task_state", None), "tool_steps", 0)
                )
                self._last_tool_result_metadata["read_range_guard"] = {
                    "path": task_policy.canonical_path(args.get("path")),
                    "requested_range": [int(args.get("start", 1)), int(args.get("end", 200))],
                    "covered_ranges": (
                        [list(item) for item in read_coverage.covered_ranges]
                        if read_coverage is not None
                        else []
                    ),
                    "returned_ranges": (
                        [list(item) for item in read_coverage.uncovered_ranges]
                        if read_range_action == "delta_read"
                        else [[int(args.get("start", 1)), int(args.get("end", 200))]]
                    ),
                    "file_changed": read_range_action == "stale_read",
                    "action": read_range_action,
                    "file_version": read_range_version,
                }
            if name == "retrieve_code" and self.last_retrieval_result is not None:
                self._last_tool_result_metadata.update(
                    {
                        "retrieval_strategy": self.last_retrieval_result.strategy,
                        "semantic_applied": self.last_retrieval_result.semantic_applied,
                        "rerank_applied": self.last_retrieval_result.rerank_applied,
                    }
                )
            if workspace_changed:
                self.invalidate_read_ranges(affected_paths)
            ...
            if tool.get("circuit_breaker", True):
                if business_success:
                    self.tool_circuit_breaker.record_success(name)
                else:
                    self.tool_circuit_breaker.record_failure(name)
            return result
        except Exception as exc:
            after_snapshot = (
                self.capture_workspace_snapshot() if tool["risky"] else before_snapshot
                ...
                self._last_tool_result_metadata.update(
                    {
                        ...
                        "side_effect_prior_state": "",
                        "side_effect_commit_recorded": False,
                        "side_effect_outcome_uncertain": True,
                        "idempotency_key_conflict": False,
                    }
                )
                self.run_store.update_side_effect_operation(
